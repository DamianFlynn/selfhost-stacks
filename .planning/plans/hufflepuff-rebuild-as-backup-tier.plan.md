...
- **Open and not addressed by this plan:** the email/PST archive hunt. Confirmed absent from hufflepuff `/` and from the scavenged copy. Candidate locations not yet searched: the intel-mac (343 GB photo library host, macOS 13.7.8), the mac-mini, M365/OneDrive via the work tenant, iCloud, and any Gmail account — the Gmail MCP connector is present but **not connected** in the session where this was attempted. `stacks/selfhosted/open-archiver/compose.yaml` already declares a full mail-archiving stack (`logiclabshq/open-archiver:v0.6.0` + Postgres 18 + Valkey + Meilisearch v1.54 + Tika) that has **never been deployed** — the archiving destination already exists in the repo and needs a plan of its own, plus a consolidation decision, since it would add a third Meilisearch, a thirteenth Postgres and a third Tika to the estate.
