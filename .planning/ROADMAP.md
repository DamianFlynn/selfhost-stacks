...
- [ ] 06-46-PLAN.md — R6-01/R6-03(a): the sole `shellcheck -S warning` finding across the six instrument scripts, and self-describing remediation output on both pinned-count failures in `check-music-freeze.sh`
- [ ] 06-47-PLAN.md — R6-05: a heading-text (never line-number) pointer at the head of `beets.md` naming the authoritative current state, plus the three-way Phase 6 disposition consistency audit
- [ ] 06-48-PLAN.md — R6-06/R6-07: `DEF-06-45-04`'s non-detecting detector corrected and DRIVEN against a control string, and the bracketing convention's scope decided — stays `artifacts/`, not widened to ROADMAP prose, not dropped
